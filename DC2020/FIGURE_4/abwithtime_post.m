function [pr_median,magco_median, bv_median, av_ann_median, timev, bprevalue,timewindow,result_flag_median,bpreplus,bpreminus]=abwithtime_post(catSpre,N,mbin,Mms,corr)
...
for j = 1:step:no_estimates_pre
    ...
    if ~isempty(b_orig)
        if length(b_orig(:,1))>=Nmin
            ...
            % estimate max curv Mc, cut catalog
            ...
            % if enough events, estimate parameters
            if N3>=Nmin
