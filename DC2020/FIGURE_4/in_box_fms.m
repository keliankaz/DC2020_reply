function [I,boxCorners,XYZ] = in_box_fms(lat,lon,depth,hlat,hlon,hdepth,hM,hstrike,hdip,hfms,W,save_out,fn,M,t)
...
% Edits (working on reply): 01/25/21
%    - adds different dimensions
...
switch hfms                     % Wells and Coppersmith, 1994 (table 2A)
    case 'SS'                   % strike slip (subsurface rupture lenght/wdith)
        aL = -2.57; bL = 0.62;
        aW = -0.76; bW = 0.27;
    case 'R'                    % reverse
        aL = -2.42; bL = 0.58;
...
length_scaling     = @(MOMENT_MAGNITUDE) 10^(aL+bL*MOMENT_MAGNITUDE);
width_scaling    = @(MOMENT_MAGNITUDE) 10^(aW+bW*MOMENT_MAGNITUDE);
LW                = width_scaling(hM);
LL                = length_scaling(hM);

if LL < W % the box is wider than it is long revert to a spherical search
    I = sum(([x,y,z]-[xh,yh,zh]).^2,2) < W^2;
    boxCorners = nan;
    warning('Box would be wider than it is long - working with a sphere instead')
else
    % make flat lying box
    xt = repmat([-LL/2,-LL/2, LL/2, LL/2],1,2)';
    yt = repmat([-LW/2, LW/2, LW/2,-LW/2],1,2)';
    ...
    XYZt = XYZt*roty(hlat-90);  % rotate with respect to the surface of
    XYZt = XYZt*rotz(-hlon);    % the earth
